let d8r = (function (d3) {
    ...
    function refreshNodeArray(nodesArray, serverResponse) {
        let currentNodesId = nodesArray.map((x) => x.nodes[0].id);

        for (let i = 0; i < serverResponse.length; i++) {
            if (currentNodesId.includes(serverResponse[i].nodes[0].id)) {
                let uNode = nodesArray.find((x) => x.nodes[0].id === serverResponse[i].nodes[0].id);
                for (let j = 0; j < uNode.links.length; j++) {
                    let emoProb = serverResponse[i].links.find((x) => x.target === uNode.links[j].target.id).value;
                    uNode.links[j].value = emoProb;
                    let emoIndex = uNode.links[j].target.index;
                    uNode.nodes[0].donut[emoIndex] = emoProb;
                }
            } else {
                let nuNode = serverResponse[i];
                nodesArray.push(nuNode);
            }
        }
        return nodesArray;
    }
